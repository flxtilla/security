...
var (
	testPublicKey  []byte
	defaultKeyFunc Keyfunc = func(t *Token) (interface{}, error) { return testPublicKey, nil }
...
func mkTokenString(c map[string]interface{}) string {
	key, e := ioutil.ReadFile("resources/sample_key")
	if e != nil {
		panic(e.Error())
	}
	token := New(SigningMethodRS256)
	token.Claims = c
	st, e := token.SignedString(key)
...
func TestToken(t *testing.T) {
	for _, data := range tokenTestData {
		if data.tokenString == "" {
			data.tokenString = mkTokenString(data.claims)
		}
		...
		if data.errors != 0 {
			if err == nil {
				...
				if err.(*ValidationError).Errors != data.errors {
...
func init() {
	var err error
	if testPublicKey, err = ioutil.ReadFile("resources/sample_key.pub"); err != nil {
		panic(err)
	}
}
